<template>
  <div class="pa-4">
    <div v-if="solicitudes.length === 0" class="text-center py-12">
      <v-icon size="80" color="grey-lighten-3">mdi-cash-refund</v-icon>
      <p class="text-h6 text-grey mt-4">No hay solicitudes de reposición</p>
      <p class="text-body-1 text-grey">No se encontraron registros</p>
    </div>

    <div v-else class="text-caption text-grey mb-3">
      Mostrando {{ solicitudes.length }} solicitudes de reposición
    </div>
    ...
    <v-table hover class="solicitudes-table" density="comfortable">
      <thead>
        <tr>
          ...
          <th class="text-left" width="180">Tipo Gasto</th>
      ...
      <tbody>
        ...
        >
          ...
          <td>
            <v-chip v-if="solicitud.tipo_gasto" size="small" color="secondary" variant="tonal">
          ...
          <td class="text-center">
            <span class="text-body-1 font-weight-bold text-secondary-darken-1">
