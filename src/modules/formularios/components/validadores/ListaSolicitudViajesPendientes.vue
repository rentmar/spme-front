<template>
  <div class="pa-4">
    <div v-if="solicitudes.length === 0" class="text-center py-12">
      <v-icon size="80" color="grey-lighten-3">mdi-airplane</v-icon>
      <p class="text-h6 text-grey mt-4">No hay solicitudes de viajes</p>
      <p class="text-body-1 text-grey">No se encontraron registros</p>
    </div>

    <div v-else class="text-caption text-grey mb-3">
      Mostrando {{ solicitudes.length }} solicitudes de viajes
    </div>
    ...
    <v-table hover class="solicitudes-table" density="comfortable">
      <thead>
        <tr>
          ...
          <th class="text-left" width="160">Destino</th>
        ...
        >
          ...
          <td>
            <div class="d-flex align-center gap-2">
              <v-icon size="20" color="info">mdi-map-marker</v-icon>
              <span class="text-body-2 font-weight-medium">{{ solicitud.destino || 'N/A' }}</span>
            </div>
          </td>
          <td>
          ...
          <td class="text-center">
            <span class="text-body-1 font-weight-bold text-info-darken-1">
