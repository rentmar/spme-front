<template>
  <div class="pa-4">
    <div v-if="solicitudes.length === 0" class="text-center py-12">
      <v-icon size="80" color="grey-lighten-3">mdi-cash-multiple</v-icon>
      <p class="text-h6 text-grey mt-4">No hay solicitudes de fondos</p>
      <p class="text-body-1 text-grey">No se encontraron registros</p>
    </div>

    <div v-else class="text-caption text-grey mb-3">
      Mostrando {{ solicitudes.length }} solicitudes de fondos
    </div>
    ...
    <v-table hover class="solicitudes-table" density="comfortable">
      <thead>
        <tr>
          ...
          <th class="text-left" width="180">Actividad</th>
      ...
      <tbody>
        ...
        >
          ...
          <td>
            <v-chip v-if="solicitud.actividad" size="small" color="primary" variant="tonal">
              {{ solicitud.actividad }}
            </v-chip>
          ...
          <td class="text-center">
            <span class="text-body-1 font-weight-bold text-success-darken-1">
