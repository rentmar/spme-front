<template>
  <div class="pa-4">
    <div v-if="solicitudes.length === 0" class="text-center py-12">
      <v-icon size="80" color="grey-lighten-3">mdi-credit-card</v-icon>
      <p class="text-h6 text-grey mt-4">No hay solicitudes de pago directo</p>
      <p class="text-body-1 text-grey">No se encontraron registros</p>
    </div>

    <div v-else class="text-caption text-grey mb-3">
      Mostrando {{ solicitudes.length }} solicitudes de pago directo
    </div>
    ...
    <v-table hover class="solicitudes-table" density="comfortable">
      <thead>
        <tr>
          ...
          <th class="text-left" width="180">Proveedor</th>
          <th class="text-left" width="140">Factura</th>
          <th class="text-left" width="180">Solicitante</th>
          <th class="text-center" width="140">Monto</th>
        ...
        >
          ...
          <td>
            <div class="d-flex align-center gap-2">
              <v-icon size="20" color="warning">mdi-store</v-icon>
              <span class="text-body-2">{{ solicitud.proveedor || 'N/A' }}</span>
            </div>
          </td>
          <td>
            <v-chip v-if="solicitud.factura" size="small" color="orange" variant="tonal">
              {{ solicitud.factura }}
            </v-chip>
          ...
          <td class="text-center">
            <span class="text-body-1 font-weight-bold text-warning-darken-1">
