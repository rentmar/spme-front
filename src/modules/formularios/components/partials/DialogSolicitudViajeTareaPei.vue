<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="deep-purple" title="Validar Solicitud de Viaje (Subactividad) PEI">
...
const { listaSolicitudViajeTareaPei, obtenerListaSolViajesPorIdTareaPei } = useSolicitudViaje()
...
const solicitudes = computed(() => {
  if (listaSolicitudViajeTareaPei.value && listaSolicitudViajeTareaPei.value.solicitudes) {
    return listaSolicitudViajeTareaPei.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolViajesPorIdTareaPei(props.actividadId, props.tareaId)

    // Extraer estadísticas, actividad y tarea de la respuesta
    if (listaSolicitudViajeTareaPei.value) {
      estadisticas.value = listaSolicitudViajeTareaPei.value.estadisticas
      actividad.value = listaSolicitudViajeTareaPei.value.actividad
      tarea.value = listaSolicitudViajeTareaPei.value.tarea
    }

    console.log('Datos cargados (viaje tarea):', listaSolicitudViajeTareaPei.value)
