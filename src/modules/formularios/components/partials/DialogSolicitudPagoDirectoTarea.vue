<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="teal-darken-2" title="Validar Solicitud de Pago Directo (Subactividad)">
          ...
          <div class="d-flex justify-space-between align-center mb-3">
            <div>
              ...
              <div class="d-flex flex-wrap align-center gap-2 mt-1">
                <v-chip size="small" color="teal-darken-2" variant="outlined">
        ...
        <div v-if="!loading">
              ...
              <div class="d-flex justify-space-between align-start">
                <div class="solicitud-info flex-grow-1">
                  <div class="d-flex align-center mb-1">
                    ...
                    <v-chip size="x-small" class="ml-2" color="teal-lighten-4">
                    ...
                    <v-expansion-panels variant="accordion" flat>
                      <v-expansion-panel>
                        <v-expansion-panel-title class="text-caption py-1">
                          <span class="font-weight-medium">Detalle destino de fondos</span>
                          <template v-slot:actions>
                            <v-icon color="teal-darken-2">mdi-chevron-down</v-icon>
                          </template>
                          ...
                          >
                            <div class="d-flex justify-space-between">
                              <div>
                                ...
                                <div class="text-grey">Partida: {{ item.partida_sf }}</div>
                              </div>
          ...
          <div v-else class="pa-8 text-center">
            ...
            <div class="text-caption text-grey mt-2">
              No se encontraron solicitudes de pago directo para esta tarea
            </div>
          </div>
        </div>
...
const { listaSolPagoDirectoTarea, obtenerListaSolPagoDirectoPorIdTarea } = useSolicitudPagoDirecto()
...
const solicitudes = computed(() => {
  if (listaSolPagoDirectoTarea.value && listaSolPagoDirectoTarea.value.solicitudes) {
    return listaSolPagoDirectoTarea.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolPagoDirectoPorIdTarea(props.actividadId, props.tareaId)

    // Extraer estadísticas, actividad y tarea de la respuesta
    if (listaSolPagoDirectoTarea.value) {
      estadisticas.value = listaSolPagoDirectoTarea.value.estadisticas
      actividad.value = listaSolPagoDirectoTarea.value.actividad
      tarea.value = listaSolPagoDirectoTarea.value.tarea
    }

    console.log('Datos cargados (pago directo tarea):', listaSolPagoDirectoTarea.value)
    console.log('Estadísticas (pago directo tarea):', estadisticas.value)
  } catch (err) {
