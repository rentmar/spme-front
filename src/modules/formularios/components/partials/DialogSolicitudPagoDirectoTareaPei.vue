<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar
        color="teal-darken-2"
        title="Validar Solicitud de Pago Directo (Subactividad) PEI "
      >
...
const { listaSolPagoDirectoTareaPei, obtenerListaSolPagoDirectoPorIdTareaPei } =
...
const solicitudes = computed(() => {
  if (listaSolPagoDirectoTareaPei.value && listaSolPagoDirectoTareaPei.value.solicitudes) {
    return listaSolPagoDirectoTareaPei.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolPagoDirectoPorIdTareaPei(props.actividadId, props.tareaId)

    // Extraer estadísticas, actividad y tarea de la respuesta
    if (listaSolPagoDirectoTareaPei.value) {
      estadisticas.value = listaSolPagoDirectoTareaPei.value.estadisticas
      actividad.value = listaSolPagoDirectoTareaPei.value.actividad
      tarea.value = listaSolPagoDirectoTareaPei.value.tarea
    }

    console.log('Datos cargados (pago directo tarea):', listaSolPagoDirectoTareaPei.value)
