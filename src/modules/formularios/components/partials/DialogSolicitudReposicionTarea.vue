<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="amber-darken-3" title="Validar Solicitud de Reposición (Subactividad)">
...
const solicitudes = computed(() => {
  if (listaSolReposicionTarea.value && listaSolReposicionTarea.value.solicitudes) {
    return listaSolReposicionTarea.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    ...
    if (listaSolReposicionTarea.value) {
      estadisticas.value = listaSolReposicionTarea.value.estadisticas
      actividad.value = listaSolReposicionTarea.value.actividad
      tarea.value = listaSolReposicionTarea.value.tarea
    }

    console.log('Datos cargados (reposición tarea):', listaSolReposicionTarea.value)
