<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="amber-darken-3" title="Validar Solicitud de Reposición">
...
const { listaSolReposicionActividad, obtenerListaSolReposicionPorIdActividad } =
...
const solicitudes = computed(() => {
  if (listaSolReposicionActividad.value && listaSolReposicionActividad.value.solicitudes) {
    return listaSolReposicionActividad.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolReposicionPorIdActividad(props.actividadId)

    // Extraer estadísticas y actividad de la respuesta
    if (listaSolReposicionActividad.value) {
      estadisticas.value = listaSolReposicionActividad.value.estadisticas
      actividad.value = listaSolReposicionActividad.value.actividad
    }

    console.log('Datos cargados (reposición):', listaSolReposicionActividad.value)
