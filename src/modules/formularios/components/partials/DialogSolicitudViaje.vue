<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="deep-purple" title="Validar Solicitud de Viaje">
...
const { listaSolicitudViajeActividad, obtenerListaSolViajesPorIdActividad } = useSolicitudViaje()
...
const solicitudes = computed(() => {
  if (listaSolicitudViajeActividad.value && listaSolicitudViajeActividad.value.solicitudes) {
    return listaSolicitudViajeActividad.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolViajesPorIdActividad(props.actividadId)

    // Extraer estadísticas y actividad de la respuesta
    if (listaSolicitudViajeActividad.value) {
      estadisticas.value = listaSolicitudViajeActividad.value.estadisticas
      actividad.value = listaSolicitudViajeActividad.value.actividad
    }

    console.log('Datos cargados (viaje):', listaSolicitudViajeActividad.value)
    console.log('Estadísticas (viaje):', estadisticas.value)
  } catch (err) {
