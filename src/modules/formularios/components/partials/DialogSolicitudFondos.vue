<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="primary" title="Validar Solicitud de Fondos">
...
const { listaSolicitudesFondosActividad, obtenerListaSolFondosPorIdActividad } =
  useSolucitudFondos()
...
const solicitudes = computed(() => {
  if (listaSolicitudesFondosActividad.value && listaSolicitudesFondosActividad.value.solicitudes) {
    return listaSolicitudesFondosActividad.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolFondosPorIdActividad(props.actividadId)

    // Extraer estadísticas y actividad de la respuesta
    if (listaSolicitudesFondosActividad.value) {
      estadisticas.value = listaSolicitudesFondosActividad.value.estadisticas
      actividad.value = listaSolicitudesFondosActividad.value.actividad
    }

    console.log('Datos cargados:', listaSolicitudesFondosActividad.value)
