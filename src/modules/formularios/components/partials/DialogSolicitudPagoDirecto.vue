<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="blue-darken-2" title="Validar Solicitud de Pago Directo">
                ...
                <div class="solicitud-info flex-grow-1">
                  ...
                  <div v-if="solicitud.datos_forma_pago" class="text-caption">
                    ...
                    >
                      ...
                      {{ solicitud.datos_forma_pago.transferencia.entidad_bancaria }} -
                    ...
                    <div v-else-if="solicitud.formaPago === 1 && solicitud.datos_forma_pago.otros">
                      ...
                      {{ solicitud.datos_forma_pago.otros.nombre_otros }}
                    </div>
                  </div>
                </div>
...
const { listaSolPagoDirectoActividad, obtenerListaSolPagoDirectoPorIdActividad } =
...
const solicitudes = computed(() => {
  if (listaSolPagoDirectoActividad.value && listaSolPagoDirectoActividad.value.solicitudes) {
    return listaSolPagoDirectoActividad.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolPagoDirectoPorIdActividad(props.actividadId)

    // Extraer estadísticas y actividad de la respuesta
    if (listaSolPagoDirectoActividad.value) {
      estadisticas.value = listaSolPagoDirectoActividad.value.estadisticas
      actividad.value = listaSolPagoDirectoActividad.value.actividad
    }

    console.log('Datos cargados (pago directo):', listaSolPagoDirectoActividad.value)
    console.log('Estadísticas (pago directo):', estadisticas.value)
  } catch (err) {
