<template>
  <v-dialog v-model="dialog" max-width="800" @after-enter="cargarDatos">
    <v-card>
      <v-toolbar color="primary" title="Validar Solicitud de Fondos (Tarea)">
...
const { lsitaSolicitudesFondosTarea, obtenerListaSolFondosPorIdTarea } = useSolucitudFondos()
...
const solicitudes = computed(() => {
  if (lsitaSolicitudesFondosTarea.value && lsitaSolicitudesFondosTarea.value.solicitudes) {
    return lsitaSolicitudesFondosTarea.value.solicitudes
  }
  return []
})
...
const cargarDatos = async () => {
  ...
  try {
    await obtenerListaSolFondosPorIdTarea(props.actividadId, props.tareaId)

    // Extraer estadísticas, actividad y tarea de la respuesta
    if (lsitaSolicitudesFondosTarea.value) {
      estadisticas.value = lsitaSolicitudesFondosTarea.value.estadisticas
      actividad.value = lsitaSolicitudesFondosTarea.value.actividad
      tarea.value = lsitaSolicitudesFondosTarea.value.tarea
    }

    console.log('Datos cargados (fondos tarea):', lsitaSolicitudesFondosTarea.value)
