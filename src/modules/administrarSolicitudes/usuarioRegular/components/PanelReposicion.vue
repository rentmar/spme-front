<!-- components/usuario/PanelReposicion.vue -->
<template>
  <div class="pa-4">
    ...
    >
      <template #item.codigo="{ item }">
        <div>
          ...
          <div class="text-caption text-grey">{{ item.tipo_gasto || '-' }}</div>
        </div>
      </template>
...
const camposFiltro = [
  ...
  { type: 'text', key: 'tipoGasto', label: 'Tipo de gasto', width: 180 },
  { type: 'number', key: 'montoMinimo', label: 'Monto mínimo', prefix: 'Bs.' },
]

const headers = [
  { title: 'Código', key: 'codigo', sortable: true, width: '150' },
  { title: 'Tipo Gasto', key: 'tipo_gasto', sortable: true, width: '180' },
