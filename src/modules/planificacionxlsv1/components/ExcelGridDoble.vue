...
<template>
  <main class="grid-area" ref="gridEl">
    ...
    <v-tabs-window v-model="localTab" class="grid-window">
      ...
      <v-tabs-window-item value="tareas" v-if="mostrarTabTareas">
        <div
        ...
        <HotTable
          v-else
          ref="tareasTable"
          :beforeChange="beforeChangeTareas"
          ...
          :contextMenu="contextMenuConfigTareas"
        />
      </v-tabs-window-item>
    </v-tabs-window>
  </main>
...
const props = defineProps({
  ...
  contextMenuConfigTareas: [Array, Object],
})
...
//Para el control de acceso de las tareas
// Para grilla de tareas (AGREGAR)
const beforeChangeTareas = (changes, source) => {
  if (source === 'loadData') return true

  for (let i = 0; i < changes.length; i++) {
    const [row, prop] = changes[i]

    // Bloquear filas vacías
    const rowId = tareasTable.value?.hotInstance?.getDataAtRowProp(row, 'id')
    if (rowId === null || rowId === undefined || rowId === '') {
      infoMsg('No se pueden editar filas vacías')
      return false
    }
    ...
    const estado = tareasTable.value?.hotInstance?.getDataAtRowProp(row, 'estado')
    if (!puedeEditar('tarea', prop, estado)) {
      infoMsg(`No tienes permiso para editar "${prop}"`)
      return false
    }
  }
...
// Debug: ver qué llega
watch(
  () => props.contextMenuConfigTareas,
  (val) => {
    console.log('contextMenuConfigTareas recibido:', val)
  },
  { immediate: true },
)
</script>
