<template>
  <v-card elevation="2" rounded="lg" class="mb-4 grupo-card">
    <!-- Header colapsable del proyecto -->
    <v-card-item
      class="pa-4 cursor-pointer grupo-header"
      :class="{ 'bg-primary-lighten-5': expandido }"
      @click="$emit('toggle')"
    >
      <template v-slot:prepend>
        <v-avatar :color="expandido ? 'primary' : 'grey-lighten-1'" size="44">
          <v-icon size="24" color="white">mdi-folder</v-icon>
        </v-avatar>
      </template>
      ...
      <v-card-title class="text-h6 font-weight-bold pa-0">
        <div>
          <div class="d-flex align-center flex-wrap">
            ...
            <span class="text-truncate">{{ proyecto.proyectoNombre }}</span>
          </div>
          <div class="text-caption text-medium-emphasis mt-1">ID: {{ proyecto.proyectoId }}</div>
        </div>
      </v-card-title>
            ...
            <v-list density="compact" class="bg-grey-lighten-5 pa-2 ml-4">
              <div class="d-flex justify-space-between align-center mb-2 px-2">
                <span class="text-caption font-weight-bold text-uppercase text-grey">
                  <v-icon size="16" color="primary" class="mr-1">mdi-playlist-check</v-icon>
...
// Navegación
const irInformeActividad = (actividadId, esPei) => {
