    ...
    <v-expansion-panels variant="accordion">
      <v-expansion-panel v-for="item in items" :key="item.id" class="mb-2">
        <v-expansion-panel-title>
          <div class="d-flex align-center">
            ...
            <v-icon :color="item.entregado ? 'green' : 'orange'">
              {{ item.entregado ? 'mdi-check-circle' : 'mdi-progress-clock' }}
            </v-icon>
          </div>
        </v-expansion-panel-title>
        ...
        <v-expansion-panel-text>
          ...
          <div v-if="item.supuestos || item.riesgos" class="mb-4">
            <v-alert v-if="item.supuestos" type="info" variant="tonal" class="mb-2">
              <strong>Supuestos:</strong> {{ item.supuestos }}
            </v-alert>

            <v-alert v-if="item.riesgos" type="warning" variant="tonal">
