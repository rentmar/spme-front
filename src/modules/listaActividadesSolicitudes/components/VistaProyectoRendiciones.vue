<template>
  <div>
    <div v-for="grupo in proyectosPaginados" :key="`p-${grupo.proyecto_id}`" class="mb-4">
      <v-card
...
const props = defineProps({ proyectos: { type: Array, default: () => [] } })
defineEmits(['accion'])
...
const proyectosPaginados = computed(() => {
  ...
  return props.proyectos.slice(s, s + pageSize)
})
const totalPages = computed(() => Math.ceil(props.proyectos.length / pageSize))
