  ...
  <v-expansion-panels multiple class="marco-logico-panels">
    <v-expansion-panel elevation="1" class="marco-logico-panel">
      <v-expansion-panel-title expand-icon="mdi-chevron-down" collapse-icon="mdi-chevron-up">
        <v-icon icon="mdi-package-variant" color="teal-darken-2" class="mr-3" />
      ...
      <v-expansion-panel-text class="panel-content">
          ...
          <v-row dense>
            ...
            <v-col cols="12" md="4">
              <v-card class="marco-logico-card">
                ...
                <v-card-text>
                  <v-expansion-panels v-model="panelProductoAbierto" multiple>
                    <v-expansion-panel
                    ...
                    >
                      ...
                      <v-expansion-panel-text>
                        ...
                        <v-btn size="small" color="primary" @click="seleccionarProducto(producto)">
                          Ver Indicadores/Actividades
                        </v-btn>
                      </v-expansion-panel-text>
                    </v-expansion-panel>
                  </v-expansion-panels>
                ...
                <v-card-text>
                  <div v-if="productoSeleccionado">
                    <v-expansion-panels multiple>
                      <template v-if="productoSeleccionado.indicadores?.length > 0">
                        <v-expansion-panel
                          v-for="(indicador, idx) in productoSeleccionado.indicadores"
                          :key="idx"
                        >
                          <v-expansion-panel-title
                            expand-icon="mdi-chevron-down"
                  ...
                  <div v-else class="text-center text-grey">
                    Selecciona un producto para ver sus indicadores.
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
              ...
              <v-card class="marco-logico-card">
                ...
                <v-card-text>
                  <div v-if="productoSeleccionado">
                    <v-expansion-panels multiple>
                      <template
                        v-if="actividadesDeProducto(productoSeleccionado.codigo).length > 0"
                      >
                        <v-expansion-panel
                          v-for="(actividad, idx) in actividadesDeProducto(
                      ...
                      <template v-else>
                        <div class="text-center text-grey">
                          No hay actividades asociadas a este producto.
                        </div>
                      </template>
                    </v-expansion-panels>
                  </div>
                  <div v-else class="text-center text-grey">
                    Selecciona un producto para ver sus actividades.
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
        ...
        <template v-else>
          <div class="text-center text-grey py-10">
            ...
            <div>No hay productos disponibles para mostrar.</div>
          </div>
        </template>
      </v-expansion-panel-text>
    </v-expansion-panel>
...
const props = defineProps({
  ...
  actividadesProductos: { type: Array, required: true },
})

const cantidadProductos = computed(() => props.productos?.length ?? 0)
const hayProductos = computed(() => Array.isArray(props.productos) && props.productos.length > 0)

const productoSeleccionado = ref(null)
const panelProductoAbierto = ref([])

const seleccionarProducto = (producto) => {
  productoSeleccionado.value = producto
}

const actividadesDeProducto = (codigoProducto) => {
  const resultado = props.actividadesProductos.find((p) => p.codigo === codigoProducto)
