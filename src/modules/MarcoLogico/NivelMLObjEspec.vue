  ...
  <v-expansion-panels multiple class="marco-logico-panels">
    ...
    <v-expansion-panel elevation="2" class="marco-logico-panel">
      <v-expansion-panel-title expand-icon="mdi-chevron-down" collapse-icon="mdi-chevron-up">
        <v-icon icon="mdi-bullseye-arrow" color="secondary" class="mr-3" />
        <span class="panel-title">Objetivo Especificos</span>
          ...
          <v-col cols="12" md="6">
            <v-card class="marco-logico-card">
              <v-card-title class="d-flex align-center">
                ...
                <span class="card-title">Objetivos Especificos</span>
              </v-card-title>

              <v-card-text>
                <v-expansion-panels v-model="panelObjetivoAbierto" multiple>
                  <v-expansion-panel
                    v-for="(objetivo, index) in props.objetivosEspecificos"
                    :key="objetivo.codigo"
...
const props = defineProps({
  objetivosEspecificos: { type: Array, required: true },
})

const cantidadObjetivos = computed(() => props.objetivosEspecificos?.length ?? 0)
