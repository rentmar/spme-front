...
export const impresionFormulariosServicio = {
  ...
  imprimirInformeActividadPrincipal: async (idInformeActividad, descargar = true) => {
    try {
      const respuesta = await apiPrint.get(
        '/informe-actividad-principal/' + idInformeActividad + '/pdf/',
        {
          responseType: 'blob',
        },
      )
      ...
      // Extraer el nombre del archivo del header Content-Disposition
      let nombreArchivo = `informe-actividad-${idInformeActividad}.pdf`
  ...
  imprimirInformeTareaPrincipal: async (idInformeTarea, descargar = true) => {
    try {
      const respuesta = await apiPrint.get('/informe-tarea-principal/' + idInformeTarea + '/pdf/', {
        responseType: 'blob',
      })
      ...
      // Extraer el nombre del archivo del header Content-Disposition
      let nombreArchivo = `informe-actividad-${idInformeTarea}.pdf`
