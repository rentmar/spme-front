...
const { generarPdfSolicitudReposicionPei, generarPdfSolicitudReposicionTareasPei } =
...
const generarPdfSolicitudFondosFunc = async () => {
  ...
  try {
    ...
    await generarPdfSolicitudReposicionPei(idSolicitud)
  } catch (error) {
...
const generarPdfSolicitudSubactividadFunc = async () => {
  ...
  try {
    await generarPdfSolicitudReposicionTareasPei(idSolicitud)
  } catch (error) {
...
watch(
  ...
  (newVal) => {
    ...
    formData.value.idresponsable = newVal.responsable
    // const getSafeValue = (value, defaultValue = '') => {
    //   return value !== null && value !== undefined ? value : defaultValue
    // }
...
function addGasto() {
  formData.value.detalle_destino_fondos.push({ partida: '', concepto: '', monto: 0 })
}
...
async function submitForm() {
  ...
  try {
    ...
    const payload = {
      ...
      usuario: usuario.value.id, //formData.value.id_usuario,
    ...
    const response = await fetch(baseurl + 'api/solicitud-reembolso-pei/' + idSolicitud + '/', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function validarSolicitud() {
  ...
  const payload = {
    id: formDatSF.value.idsf || idSolicitud, // Usar idsf o idSolicitud de la URL
    ...validacionData, // Incluye solo validacion_responsable O validacion_coordinador
  }

  console.log('00000000000000000000000000000', JSON.stringify(payload, null, 2))
  ...
  try {
    const response = await fetch(
      ...
      baseurl + 'api/solicitud-reembolso-pei/actualizar_estado/',
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
...
function resetForm() {
  Object.assign(formData.value, {
    descripcion_actividad: '',
    objetivo_actividad: '',
    detalle_destino_fondos: [{ partida: '', concepto: '', monto: 0 }],
    forma_pago: '',
    lugar_solicitud: '',
    fecha_solicitud: getCurrentDate(),
    monto_solicitado: 0,
...
function actualizarDatosFormulario(solicitud) {
  ...
  formDatSF.value.formaPago_idsf = solicitud.formaPago || null
  ...
  formDatSF.value.responsable_idsf = solicitud.responsable || null
  formDatSF.value.validacionCoordinadorsf = solicitud.validacionCoordinador || false
  formDatSF.value.coordinador_idsf = solicitud.coordinador || null
  formDatSF.value.usuario_idsf = solicitud.usuario || null
  formDatSF.value.actividad_idsf = solicitud.actividad || null
