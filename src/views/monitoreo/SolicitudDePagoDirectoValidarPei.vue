<template>
  <v-container class="solicitud-fondos-container">
    ...
    <div v-if="!cargandoGeneral">
      <!--Titulo de la pagina-->
      <PaginaTituloIcono
        :titulo="'Validar Solicitud de Pago Directo PEI'"
        :icon="'mdi-cash-check'"
      ></PaginaTituloIcono>
...
async function validarSolicitud() {
  ...
  loading.value = true
  try {
    const response = await fetch(baseurl + 'api/solicitud-pago-directo-pei/actualizar_estado/', {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
      },
...
function actualizarDatosFormulario(solicitud) {
  ...
  formDatSF.value.responsable_idsf = solicitud.contador || null
