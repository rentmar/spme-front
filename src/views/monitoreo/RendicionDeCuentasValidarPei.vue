<template>
    ...
    <div v-if="!cargandoGeneral">
      <PaginaTituloIcono
        :titulo="'Validar Rendición de Cuentas PEI'"
  ...
  console.log('Ruta:', JSON.stringify(route, null, 2))
  ...
  async function cargarDatos() {
    ...
    try {
      ...
      console.log('hhhhhhhhhhhhhhhhhhhhhhh:', datosFormulario.value)
    } catch (err) {
  ...
  async function cargarRendicionesDeCuenta() {
    ...
    try {
      ...
      console.log('wewewewewew:', JSON.stringify(rawData, null, 2))
    } catch (err) {
      error.value = err.message
      console.error('Ha ocurrido un error:', err)
    } finally {
  ...
  watch(
    ...
    (newVal) => {
      if (newVal) {
        ...
        formData.value.lugar_solicitud = newVal.lugarActividad
  ...
  async function validarRendicion(tipoValidador) {
    ...
    console.log('Payloadddddddddd:', JSON.stringify(payload, null, 2))
    // Ejecutar la llamada PATCH
    loading.value = true;
    try {
      const response = await fetch(
        baseurl+'api/rendicion-cuentas-pei/actualizar_estado/',
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
