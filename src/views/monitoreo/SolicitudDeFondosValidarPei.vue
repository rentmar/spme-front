<template>
  ...
  async function validarSolicitud() {
    ...
    const id = formDatSF.value.idsf || idSolicitud
  
    // 4. Ejecutar la llamada PATCH
    loading.value = true
    try {
      const response = await fetch(`${baseurl}api/solicitud-fondos-pei/${id}/`,
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
  ...
  function actualizarDatosFormulario(solicitud) {
    ...
    formDatSF.value.formaPago_idsf = solicitud.formaPago || null                          //clave para formulario validar
    ...
    formDatSF.value.responsable_idsf = solicitud.contador || null                         //clave para formulario validar
    formDatSF.value.validacionCoordinadorsf = solicitud.validacionCoordinador || false
    formDatSF.value.coordinador_idsf = solicitud.coordinador || null                      //clave para formulario validar
  ...
  const puedeValidarResponsable = computed(() => {
    ...
    const responsableAsignadoId = datosFormulario1.value?.contador //se cambio esta seccion para habilitar validadores
