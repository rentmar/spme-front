          ...
          <v-card elevation="2" rounded="lg" class="mb-4">
            ...
            <v-card-text class="pa-4">
              <div class="info-item mb-3">
                <div class="text-subtitle-2 text-medium-emphasis">Solicitante:</div>
                <div class="text-body-1 font-weight-medium">
                  {{ solicitante || 'No disponible' }}
                </div>
              </div>
              <div class="info-item mb-3">
                <div class="text-subtitle-2 text-medium-emphasis">Evento:</div>
                <div class="text-body-1 font-weight-medium">
                  {{ formData.evento || 'No especificado' }}
                </div>
              </div>
              <div class="info-item mb-3">
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" md="6" class="d-flex align-center">
                      <v-checkbox
                        ...
                        :disabled="
                          !puedeValidarResponsable || solicitudDeViaje.validacionResponsable
                        "
                        :readonly="
                          !puedeValidarResponsable || solicitudDeViaje.validacionResponsable
                        "
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" md="6" class="d-flex align-center">
                      <v-checkbox
                        ...
                        :disabled="
                          !puedeValidarCoordinador || solicitudDeViaje.validacionCoordinador
                        "
                        :readonly="
                          !puedeValidarCoordinador || solicitudDeViaje.validacionCoordinador
                        "
...
async function cargarSolicitudesDeViaje() {
  ...
  try {
    ...
    formData.value.instituciones_participantes =
      solicitudDeViaje.value.institucionesParticipantes || ''
...
async function validarViaje(tipoValidador, idValidador, ListaValidadores) {
  ...
  try {
    ...
    const urlForm = `${window.location.origin}/monitoreo/formulario055/${formData.value.id_actividad}?solicitud_id=${data.id}${formData.value.id_tarea ? `&tarea_id=${formData.value.id_tarea}` : ''}`
    ...
    const probarEnvioEmail = async () => {
      ...
      const datosAprobacion = {
        ...
        datos_aprobacion: {
          ...
          url_detalles: `${window.location.origin}/monitoreo/formulario055/${formData.value.id_actividad}?solicitud_id=${data.id}${formData.value.id_tarea ? `&tarea_id=${formData.value.id_tarea}` : ''}`,
        },
      }
